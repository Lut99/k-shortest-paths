...
mod tests {
    ...
    fn test_yen_ksp() {
        ...
        for _ in 0..10 {
            ...
            assert_eq!(yen(&g, "Amsterdam", "Berlin", 1), vec![path!(crate : g, "Amsterdam" -| "Berlin")]);
            assert_eq!(yen(&g, "Amsterdam", "Dorchester", 1), vec![path!(crate : g, "Amsterdam" -| "Dorchester")]);
            assert_eq!(yen(&g, "Amsterdam", "Chicago", 1), vec![path!(crate : g, "Amsterdam" -> "Dorchester" -| "Chicago")]);
            assert_eq!(yen(&g, "Berlin", "Chicago", 1), vec![path!(crate : g, "Berlin" -> "Amsterdam" -> "Dorchester" -| "Chicago")]);
        }
    }
}
...
#[track_caller]
fn yen<'g>(&mut self, graph: &'g Graph, src: &str, dst: &str, k: usize) -> Vec<Path<'g>> {
    ...
    for i in 1..k {
        ...
        for hop in 0..shortest[i - 1].hops.len() {
            ...
            let path: Path<'g> =
                Path { hops: prefix.into_iter().copied().chain(suffix.hops.into_iter().map(|(n, c)| (n, prefix.last().unwrap().1 + c))).collect() };
